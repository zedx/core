...
<div class="modal fade confirmationDialog" id="confirmEditAction" role="dialog" aria-labelledby="confirmEditLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <script type="x-tmpl-mustache" id="menuRouteEditTemplate">
      <form action="{{ route('zxadmin.menu.update', '') }}/@{{id}}" class="form-horizontal" method="POST" >
        <input name="_method" type="hidden" value="PATCH">
        <input name="_token" type="hidden" value="{{ csrf_token() }}">
        <div class="modal-body">
          <div class="form-group">
            <label class="col-sm-2 control-label">{!! trans('backend.menu.route.route') !!}</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" name="link" value="@{{link}}">
            </div>
          </div>
          <div class="form-group">
            <label class="col-sm-2 control-label">{!! trans('backend.menu.route.label') !!}</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" name="name" value="@{{name}}">
            </div>
          </div>
          <div class="form-group">
            <label class="col-sm-2 control-label">{!! trans('backend.menu.route.title') !!}</label>
            <div class="col-sm-10">
              <input type="text" class="form-control" name="title" value="@{{title}}">
            </div>
          </div>
          <div class="form-group">
            <label class="col-sm-2 control-label">{!! trans('backend.menu.route.icon') !!}</label>
            <div class="col-sm-10">

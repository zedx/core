...
class CategoriesFieldsSeeder extends Seeder
{
    ...
    {
        $price = $this->createField('Prix', 4, 'Prix immobilier', '{currency}', 1, 1, 1, 1, 0);
        $price->search()->create(['min' => 0, 'max' => 1000000, 'step' => 10000]);
        $surface = $this->createField('Surface', 4, 'Surface immobilier', 'm²', 0, 1, 1, 1, 1);
    ...
    {
        $price = $this->createField('Prix', 4, 'Prix voiture', '{currency}', 1, 1, 1, 0);
        $price->search()->create(['min' => 0, 'max' => 250000, 'step' => 10000]);
        $year = $this->createField('Année modèle', 4, 'Année modèle voiture', '', 0, 1, 1, 1);
    ...
    protected function createField($name, $type, $title, $unit, $is_price, $is_in_ad, $is_in_search, $is_in_ads_list, $is_format = 0)
    {
        return Field::create([
            'name'           => $name,
            'type'           => $type,
